...
struct Robot
{
	int rid, tid, xpos, ypos, direction, travelled;
public:
	//constructor for a robot
	Robot(const unsigned int &rid, const unsigned int &tid, int &xpos, int &ypos, int direction):
		rid(rid), tid(tid), xpos(xpos), ypos(ypos), direction(direction)
	{
		travelled = 0;
	}
};
